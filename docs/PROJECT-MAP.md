...
## `packages/client` — состав (T-007, T-008, T-017)
...
| `src/net/instanceSession.ts` | `connectInstance(url, room, world)` — адаптер Colyseus SDK: join комнаты, `sync()` перекладывает `state.players` в `WorldStore`, `sendMove(dx, dy)` шлёт `intent.move`. С T-017 — `sendAbility(abilityId)` / `sendDodge(dir)` (`intent.ability`, `intent.dodge`, форматы payload — `messages.ts` сервера). Кулдауны из state адаптер НЕ читает: `readyAtMs` — абсолютная метка серверных часов, общей временной базы с клиентом нет. Schema состояния берётся из `@game/server-instance/state` (subpath-экспорт, чтобы не тянуть серверный код в бандл) |
| `src/game-core/combat.ts` | `CombatController` (T-017) — боёвка ввода на чистом TS (TECH-SPEC 4): `pressAbility`/`pressDodge` с локальным кулдауном для мгновенного фидбека (нажатие в кулдауне блокируется локально, без round-trip), `isReady`/`readyAtMs` для HUD (T-018). Кулдаун считается строго от нажатия на инжектированных локальных часах; серверные `readyAtMs` в эту шкалу не подмешиваются (отставание часов клиента блокировало бы кнопку дольше серверного кулдауна), авторитет — у сервера: он отклоняет лишний интент сам. Возврат синхронизации шкалы — отдельное решение под HUD. `DODGE_KEY = 'dodge'` зеркалит серверный `DODGE_COOLDOWN_KEY` |
...
| `src/main.ts` | Бутстрап: сборка слоёв, клавиатурный ввод WASD/стрелки → `MovementController`, цикл rAF (setTimeout-фолбэк для скрытой вкладки): `sync` → `onAuthoritative` → `update(dt)` → `setLocalPosition`; URL сервера `?server=...` (дефолт `ws://127.0.0.1:2600`), dev-хуки `window.__gameWorld` / `__gameSync` / `__gameSetDirection` / `__gameCombat`. С T-017: боёвка клавиш (`KeyJ` → `rust-jab`, `KeyK` → `marker-shot`, `Space` → уклонение в последний ненулевой вектор ввода, autorepeat игнорируется), конфиги кулдаунов читаются fetch-ом из `/content` (`/abilities/*.json`, `/mechanics/dodge.json`) и валидируются парсерами shared |
...
| `test/combat.test.ts` | Стаб приёмки T-017 (`tsx --test`, без браузера, по образцу `movement.test.ts`): атака блокируется до истечения локального кулдауна и отправляет интент после, независимость кулдаунов способностей, отклонение неизвестного `abilityId`, уклонение с нулевым вектором не расходует кулдаун, метка готовности = нажатие + конфиг и больше ниоткуда (серверные времена в локальную шкалу не входят) |
| `index.html`, `vite.config.ts` | Дев-обвязка: `npm run dev -w @game/client`; `publicDir` указывает на корневой `/content`, чтобы боёвка-конфиги читались клиентом через fetch, а не дублировались в коде (TECH-SPEC 6) |

Проверка приёмки T-007 (вручную, браузеров в тестах нет): страница `http://localhost:5173` показывает персонажа; node-клиент, зашедший вторым, появляется в roster браузера и исчезает при leave. Проверка T-008: в браузере предсказание сдвигает персонажа в том же кадре ввода; после остановки авторитетная позиция стороннего node-обсервера совпадает с предсказанной (дрейфа нет). Проверка T-017 — автоматическая (`test/combat.test.ts`); вручную: `KeyJ` в кулдауне не отправляет интент, после 900 мс — уходит (локальный таймер, серверные `readyAtMs` в нём не участвуют), `Space` смещает авторитетную позицию на `distance` из `/content/mechanics/dodge.json`.

## `packages/server-instance` — состав (T-005, T-006, T-010, T-014, T-015, T-016, T-020)
...
| Модуль | Содержимое |
|---|---|
| `src/state.ts` | Schema-классы состояния комнаты на новом API `schema()`/`t.*` (schema 5.x, без decorators): `PlayerState` (`x`, `y`, `hp` — старт из `maxHp` конфига класса `/content/classes` (T-020, ранее серверная константа); `cooldowns` — `MapSchema<AbilityCooldownState>` с ключом по abilityId), `AbilityCooldownState` (`abilityId`, `readyAtMs` — реплицированный аналог `AbilityCooldown` из shared, T-014), `MobState` (T-015: `entityId`/`mobId`, `x`, `y`, `hp`, `targetId` — `sessionId` игрока или `''`; `statuses` — `MapSchema<StatusEffectState>` с ключом по id статуса, T-016), `StatusEffectState` (`statusId`, `damageMultiplier`, `expiresAtMs` — реплицированный аналог `StatusEffect` из shared, T-012/T-016), `InstanceState` (`players: MapSchema<PlayerState>` с ключом `sessionId`, `mobs: MapSchema<MobState>` с ключом `RoomEntityId` вида `rust-scout#1`), `SPAWN_POINT` — начальная позиция (`Vector2` из shared) |
| `src/mobAi.ts` | Чистый шаг AI одного спавна `stepMob(mob, config, players, runtime, nowMs, dtMs, damage)` (T-015, баланс из конфига — T-020): выбор ближайшего живого игрока в `aggroRadius` из конфига, липкая цель с гистерезисом — удержание между `aggroRadius` и `resetRadius`, потеря за `resetRadius`, а также при leave и смерти цели (`hp <= 0`); на мёртвого игрока моб не перецеливается (респавна нет). Сближение по `moveSpeed` без перелёта через цель, удар раз в `attackIntervalMs` на `damage` из конфига в радиусе `attackRange` — в модуле нет ни одного числа баланса. Сам удар hp не трогает: момент удара модуль отдаёт в инжектированный `DamageApplier` (T-016), типом урона служит фракция моба (отдельного поля в `MobConfig` нет, ось аффинити пока только фракционная). Время входит параметрами — юнит-тест без таймеров; `MobRuntime` (готовность удара) не реплицируется. Снятие мёртвых спавнов из state — следующий шаг T-016 |
| `src/damage.ts` | Единое применение урона (T-016): `applyDamage(request, deps)` опускает HP цели и публикует `DamageInstance` через `deps.publish`, множитель входящего урона берётся из неистёкших `MobState.statuses` (максимум, а не сумма — перезапись метки не складывается), время — `deps.now()`. `createDamageApplier(deps)` привязывает часы и канал комнаты, этот `DamageApplier` получает и `stepMob`. `DamageTarget` — игрок (`sessionId`) или спавн (`RoomEntityId`) вместе со своим state-объектом; `applyStatus`/`expireStatuses` живут на тех же часах: срок `expiresAtMs = now + durationMs`, снятие — боевым тиком |
...
| `src/classCatalog.ts` | `loadClassCatalog(dir?)` — чтение `content/classes/*.json` через `parseClassConfig` (shared), Map `classId → ClassConfig` (T-020). Отсюда комната берёт стартовые HP игрока: с T-004 персонажная система есть (`characters.class_id`), но инстанс про персонажей ничего не знает — входят по `sessionId`, а `maxHp` читается из класса-заглушки `melee-initiate` |
...
| `src/rooms/baseInstanceRoom.ts` | `BaseInstanceRoom` — комната инстанса: жизненный цикл `onCreate`/`onJoin`/`onLeave`/`onDispose` (join добавляет игрока на спавне, leave удаляет; reconnect/auth вне скоупа Фазы 0) + обработчики намерений: `intent.move` (T-006) авторитетно применяет смещение; `intent.ability` (T-014) сверяет `abilityId` с каталогом `/content` и проверяет кулдаун — до истечения отклонение без побочных эффектов, при принятии `readyAtMs = now + cooldownMs` попадает в state, а реализация — T-016: сервер сам ищет цель (ближайший живой спавн в `range` из конфига, `abilityId` на wire — единственное, что прислал клиент) и промах отклоняет без урона и статуса, кулдаун при этом уже израсходован; `intent.dodge` (T-014) — рывок на `distance` и кулдаун `cooldownMs` из конфига механики `/content/mechanics/dodge.json` (T-020). Единый механизм `startCooldown` держит и способности, и уклонение (ключ `DODGE_COOLDOWN_KEY`). С T-015 комната — ещё и хост боевого тика: при `onCreate` спавнятся мобы из `MOB_SPAWNS` (hp из `MobConfig`), `clock.setInterval` с `MOB_TICK_MS = 100` (инженерная частота дискретизации, не баланс) гоняет чистый `stepMob` с фиктивным `dt` по частоте тика и снимает истёкшие статусы (`expireStatuses`), join выдаёт игроку `maxHp` класса-заглушки `PLAYER_CLASS_ID` из `/content/classes` (T-020). Урон — через `createDamageApplier` (T-016): HP цели падает и `event.damage` расходится `broadcast`-ом в одном-единственном месте, оттуда же бьют мобы. Время комнаты — инжектируемые часы `BaseInstanceRoom.now` (дефолт `Date.now`), прокинутые через класс по той же причине, что и `logLevel`: комнату создаёт матчмейкер. Все события и отказы идут через логгер с контекстом `instanceId` + `sessionId` (T-010); уровень Colyseus передаёт через статическое поле, т.к. комнату создаёт матчмейкер |
...
| `test/instanceRoom.test.ts`, `test/intentMove.test.ts`, `test/intentAbility.test.ts`, `test/mobAi.test.ts`, `test/mobAggro.test.ts`, `test/damage.test.ts`, `test/helpers.ts` | Интеграционные тесты (node:test + `@colyseus/sdk`, сервер на порту 0): join/leave видимость в state; `intent.move` доходит до остальных, невалидный payload отклоняется; `intent.ability`/`intent.dodge` — кулдаун-цикл (отклонение до истечения, принятие после), отказ неизвестного id/мусора без падения комнаты, длина рывка сверяется с `DodgeConfig` (T-020); `mobAggro` — моб в state на старте, агро/урон по state игрока (стартовый hp — `maxHp` класса из `/content/classes`), сброс цели за `resetRadius` из конфига; `mobAi` — чистый юнит шага AI с фиктивными временем/`dt`: гистерезис агро (удержание между радиусами, потеря за `resetRadius`), запрет цели на игроке с `hp <= 0` (T-020) и удар через настоящий `DamageApplier`; `damage` — приёмка T-016 на живой комнате с подменёнными часами комнаты (`BaseInstanceRoom.now`): один и тот же входящий урон до метки и под `vulnerable` даёт разный hp, метка сгорает ровно по `durationMs` (уходит из state, множитель перестаёт действовать), урон по игроку идёт через то же применение урона (`event.damage` сходится с просадкой hp), промах вне `range` — без урона, статуса и события, но с израсходованным кулдауном; `waitFor` — опрос асинхронного state sync |
...
Классы (`classes/`, T-020): на сегодня один `melee-initiate` — заглушка, нужная чтобы стартовые HP игрока приходили из `/content`, а не из константы комнаты. С T-004 аккаунт связан с персонажами, и у каждого есть конкретный `ClassId` (`characters.class_id`, миграция 0003, значение проверяется сервисом по этому каталогу). Инстанс этого не видит: комната знает только `sessionId` вошедшего и стартовые HP берёт из `melee-initiate` — персонаж в инстанс пока не переносится.
