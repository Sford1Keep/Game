...
| `src/net/instanceSession.ts` | `connectInstance(url, room, world)` — адаптер Colyseus SDK: join комнаты, `sync()` перекладывает `state.players` в `WorldStore`, `sendMove(dx, dy)` шлёт `intent.move`. С T-017 — `sendAbility(abilityId)` / `sendDodge(dir)` (`intent.ability`, `intent.dodge`, форматы payload — `messages.ts` сервера) и `localCooldowns()` — копия кулдаунов локального игрока из авторитетного state. Schema состояния берётся из `@game/server-instance/state` (subpath-экспорт, чтобы не тянуть серверный код в бандл) |
| `src/game-core/combat.ts` | `CombatController` (T-017) — боёвка ввода на чистом TS (TECH-SPEC 4): `pressAbility`/`pressDodge` с локальным кулдауном для мгновенного фидбека (нажатие в кулдауне блокируется локально, без round-trip), `isReady`/`readyAtMs` для HUD (T-018), `applyServerCooldowns` — реконсиляция: авторитетный `readyAtMs` из state только продлевает локальный max-ом, сервер остаётся единственной инстанцией, принимающей интенты. `DODGE_KEY = 'dodge'` зеркалит серверный `DODGE_COOLDOWN_KEY`; часы инжектируются в шкале `Date.now()` (та же, что у серверного `readyAtMs`) |
...
Проверка приёмки T-007 (вручную, браузеров в тестах нет): страница `http://localhost:5173` показывает персонажа; node-клиент, зашедший вторым, появляется в roster браузера и исчезает при leave. Проверка T-008: в браузере предсказание сдвигает персонажа в том же кадре ввода; после остановки авторитетная позиция стороннего node-обсервера совпадает с предсказанной (дрейфа нет). Проверка T-017 — автоматическая (`test/combat.test.ts`); вручную: `KeyJ` в кулдауне не отправляет интент, после 900 мс — уходит, серверный `readyAtMs` виден в `session.localCooldowns()`, `Space` смещает авторитетную позицию на `distance` из `/content/mechanics/dodge.json`.
